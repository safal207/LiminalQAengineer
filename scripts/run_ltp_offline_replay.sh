...
LTP_INSPECT_FREEZE_CLOCK=1 LTP_BUILD=${ltp_sha} pnpm exec ts-node tools/ltp-inspect/inspect.ts trace --strict --quiet --format json --color never --profile agents --replay-check --input ${trace_path}
LTP_BUILD=${ltp_sha} pnpm exec ts-node tools/ltp-inspect/inspect.ts replay --color never --input ${trace_path}
LTP_BUILD=${ltp_sha} pnpm exec ts-node tools/ltp-inspect/inspect.ts replay --color never --input ${trace_path}
LTP_BUILD=${ltp_sha} pnpm exec ts-node tools/ltp-inspect/inspect.ts explain --color never --input ${trace_path} --at step-008
EOF
...
(
  ...
  LTP_INSPECT_FREEZE_CLOCK=1 LTP_BUILD="${ltp_sha}" \
    pnpm exec ts-node tools/ltp-inspect/inspect.ts trace --strict --quiet --format json --color never \
