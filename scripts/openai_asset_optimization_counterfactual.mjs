...
async function main(){
  ...
  for(const [v,runs] of Object.entries(variants)){if(runs.some(x=>x.navigation_error||!Number.isFinite(x.metrics.lcp_ms)))throw new Error(`${v} navigation or LCP collection failed`);if(v==="optimized_asset"&&runs.some(x=>x.injection.error))throw new Error("optimized interception raised an error");}
  ...
  const effect=Object.fromEntries(keys.map(k=>{const left=b[k],right=t[k];return[k,{delta:Number.isFinite(left)&&Number.isFinite(right)?round3(right-left):null,improvement_percent:Number.isFinite(left)&&Number.isFinite(right)&&left!==0?round3((left-right)/left*100):null}]}));
  const baselineObserved=variants.baseline.filter(x=>Number.isFinite(x.metrics.asset_start_ms)).length; const baselineMatch=variants.baseline.filter(x=>x.metrics.lcp_entry?.url===exp.hero_url).length; const optimizedFulfilled=variants.optimized_asset.filter(x=>x.injection.fulfilled===1).length;
  const valid=baselineObserved>=2&&baselineMatch>=2&&optimizedFulfilled>=2; const supported=valid&&effect.lcp_ms.delta<=-1000&&effect.lcp_ms.improvement_percent>=20&&prep.byte_reduction_percent>=50;
  const packet={schema_version:"liminalqa-openai-asset-counterfactual-v1",experiment:exp,verdict:!valid?"INCONCLUSIVE":supported?"SUPPORTED":"NOT_SUPPORTED",confidence:valid?"HIGH":"LOW",prepared_asset:prep,baseline_asset_observed_count:baselineObserved,baseline_lcp_match_count:baselineMatch,optimized_fulfilled_count:optimizedFulfilled,variants:{baseline:{run_count:3,medians:b,runs:variants.baseline},optimized_asset:{run_count:3,medians:t,runs:variants.optimized_asset}},effects:effect,generated_at:new Date().toISOString()};
  ...
  const md=["# LiminalQA · OpenAI homepage exact-asset optimization counterfactual","",`**Verdict:** ${packet.verdict}  `,`**Baseline asset observed:** ${baselineObserved}/3  `,`**Baseline LCP asset match:** ${baselineMatch}/3  `,`**Optimized fulfillment:** ${optimizedFulfilled}/3  `,`**Prepared bytes:** ${prep.original_bytes} → ${prep.optimized_bytes} (${prep.byte_reduction_percent}% reduction)`,"","| Metric | Baseline | Optimized | Delta | Improvement |","|---|---:|---:|---:|---:|",...keys.map(k=>`| ${k} | ${b[k]} | ${t[k]} | ${effect[k].delta} | ${effect[k].improvement_percent}% |`),"","> One exact public asset fetch and browser-local request fulfillment only. No server state, authentication, API calls, prompts, agents, fuzzing, load testing, or vulnerability claims.",""]; await fs.writeFile(path.join(rd,"summary.md"),md.join("\n")); console.log(md.join("\n"));
}
